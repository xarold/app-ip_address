...
// 
  ...
  if (!cidr.isValid()) {
    ...
    //My code to calculate IPv4 mapped IPv6 address
  ...
  //  [firstIpAddress].concat(firstIPv6Address);
  ...
  return callback(IPv4IPv6, callbackError);
}
...
function main() {
  ...
  for (let i = 0; i < sampleCidrsLen; i++) {
    ...
    getFirstIpAddress(sampleCidrs[i], (data, error) => {
      ...
      //Stringifies the object (dictionary) for formatting
